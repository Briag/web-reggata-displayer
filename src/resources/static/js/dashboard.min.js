...
app.config(function($routeProvider, $locationProvider) {
	...
	$routeProvider.when('/connect', {templateUrl: 'templates/connexion.html'});
	...
	$routeProvider.when('/boat/edit/:id', {templateUrl: 'templates/boat/edit.html'});
	...
	$routeProvider.when('/teammate/add', {templateUrl: 'templates/teammate/add.html'});
	$routeProvider.when('/teammate/edit/:id', {templateUrl: 'templates/teammate/edit.html'});
	$routeProvider.when('/notice/display/', {templateUrl: 'templates/notice/display.html'});
	$routeProvider.when('/notice/teamcomposition/:id', {templateUrl: 'templates/notice/teamcomposition.html'});
...
app.factory('authorization', function($http) {
	...
	return {
		login: function(credentials) {
			return $http.post('../../rest/auth', credentials);
		},
...
app.controller('Auth', function($scope, $location, $cookieStore, authorization, api) {
	...
	$scope.logout = function() {
		...
		var success = function(data) {
			...
			$location.path('/notice/display/');
		};
		
		authorization.logout(credentials).success(success);
	};
...
app.controller('BoatCtrl', ['$scope', 'Boat', '$cookieStore', '$location', '$route', BoatCtrl]);

function BoatCtrl($scope, Boat, $cookieStore, $location, $route) {
...
/**
 * Boat Edit Controller
 */

app.controller('BoatEdit', ['$scope', 'Boat', 'alertService', '$cookieStore', '$routeParams',  BoatEdit]);

function BoatEdit($scope, Boat, alertService, $cookieStore, $routeParams) {
 
    $scope.boat = Boat.get({id: $routeParams.id});
    
    $scope.saveProperties = function(){
    	$scope.boat.$update();
        alertService.prepForBroadcast("Modifications sauvegardées","succes");
    };
...
app.controller('TeamCtrl', ['$scope', 'Team', 'Boat', '$cookieStore', '$location', '$route', TeamCtrl]);

function TeamCtrl($scope, Team, Boat, $cookieStore, $location, $route) {
    ...
    $scope.addTeam = function() {
    	...
    	if($scope.form.name == null) {
    		...
    		for (var i=0; i<$scope.teams.length; i++) {
    			var currentTeam = $scope.teams[i];
    			if(angular.equals(angular.lowercase(currentTeam.name), angular.lowercase($scope.form.name))) {
    				found = true;
    				console.log("true");
    			}
    		}
    		if(found == false) {
    			...
    			var team = new Team();
    			team.name = $scope.form.name;
    			team.description = null;

    			if($scope.form.boat == null){
    				console.log("no boat");
    			}else{
    				team.boat = $scope.form.boat;
    				team.$save($location.path('/team/manage'));
    			}
...
app.controller('TeammateCtrl', ['$scope', 'Teammate', '$cookieStore', '$location', '$route', TeammateCtrl]);

function TeammateCtrl($scope, Teammate, $cookieStore, $location, $route) {
 
	$scope.teammates = Teammate.query();
    
	$scope.addTeammate = function() {
    	
    	if($scope.form.lastname == null || $scope.form.firstname == null) {
    		
    	} else {
    		var found = false;
    		
    		for (var i=0; i<$scope.teammates.length; i++) {
    			var currentTeammate = $scope.teammates[i];
    			
    			if(angular.equals(angular.lowercase(currentTeammate.lastname), angular.lowercase($scope.form.lastname))
    					&& angular.equals(angular.lowercase(currentTeammate.firstname), angular.lowercase($scope.form.firstname))) {
    				found = true;
    				console.log("true");
    			}
    		}
    		...
    		if(found == false) {
    			...
    			teammate.lastname = $scope.form.lastname;
    			teammate.firstname = $scope.form.firstname;
    			
    			teammate.$save($location.path('/teammate/manage'));
...
app.controller('TeammateEdit', ['$scope', 'Teammate', 'alertService', '$cookieStore', '$routeParams',  TeammateEdit]);

function TeammateEdit($scope, Teammate, alertService, $cookieStore, $routeParams) {
 
    $scope.teammate = Teammate.get({id: $routeParams.id});
    
    $scope.saveProperties = function(){
    	$scope.teammate.$update();
        alertService.prepForBroadcast("Modifications sauvegardées","succes");
    };
...
app.controller('RegattaCtrl', ['$scope', 'Regatta', '$cookieStore', '$location', '$route', RegattaCtrl]);

function RegattaCtrl($scope, Regatta, $cookieStore, $location, $route) {
 
    $scope.regattas = Regatta.query();
    
    $scope.addRegatta = function() {
    	...
    	if($scope.form.name == null) {
    		...
    		for (var i=0; i<$scope.regattas.length; i++) {
    			var currentRegatta = $scope.regattas[i];
    			if(angular.equals(angular.lowercase(currentRegatta.name), angular.lowercase($scope.form.name))) {
    				found = true;
    				console.log("true");
    			}
    		}
    		if(found == false) {
    			...
    			regatta.name = $scope.form.name;
        		
    			regatta.$save($location.path('/regatta/manage'));
    ...
    $scope.removeRegatta = function(regatta) {
    	 regatta.$delete();  
    	 $route.reload();
    }
 ...
 * Notice controller
 */
app.controller('NoticeCtrl', ['$scope', 'Regatta', '$cookieStore', NoticeCtrl]);

function NoticeCtrl($scope, Regatta, $cookieStore) {
...
/**
 * TeamComposition view Controller
 */

app.controller('TeamCompositionCtrl', ['$scope', 'Team', 'alertService', '$cookieStore', '$routeParams', TeamCompositionCtrl]);

function TeamCompositionCtrl($scope, Team, alertService, $cookieStore, $routeParams) {
